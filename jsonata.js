 ...
 */
 var jsonata = (function() {
    ...
    var parser = function (source, frm) {
        var node;
        var lexer;
        ...
        var advance = function (id, infix, object) {
            if (id && node.id !== id) {
                var code;
                if(node.id === '(end)') {
                ...
                if(object.rhs.type == "name"){
                   var proposals = ["=", ">", "<"];
                } else if(object.rhs.type == "binary"){
                    var proposals = frm ? frm.getProposals() : [];
                    proposals.push("<const>"); 
                } else {
                    var proposals = frm ? frm.getProposals() : undefined;
                }
                throw {
                    code: code,
                    stack: (new Error()).stack,
            ...
            switch (expr.type) {
                ...
                default:
                    ...
                    throw {
                        ...
                        position: expr.position,
                        token: expr.value,
                        proposals: frm ? frm.getProposals() : undefined
                    };
            }
            return result;
        };
    ...
    async function evaluate(expr, input, environment) {
        ...
        switch (expr.type) {
            case 'path':
                result = await evaluatePath(expr.steps, input, environment);
            ...
            case 'unary':
                ...
                break;
            case 'name':
                result = await evaluateName(expr, input, environment);
                break;
            case 'literal':
                result = await evaluateLiteral(expr, input, environment);
                break;
            case 'wildcard':
                result = await evaluateWildcard(expr, input, environment);
                break;
            case 'descendant':
        ...
        if (expr.hasOwnProperty('predicate')) {
            result = await applyPredicates(expr.predicate, result, environment);
    ...
    async function evaluatePath(expr, input, environment) {
        ...
        for(var ii = 0; ii < expr.length; ii++) {
            ...
            if(ii === 0 && step.consarray) {
                ...
                resultSequence = await evaluateStep(step, inputSequence, environment);
            }
     ...
     * @returns {*} Evaluated input data
     */
    async function evaluateStep(expr, input, environment) {
        var result = [];
     ...
     */
    async function applyPredicates(predicates, input, environment) {
        ...
        for(var ii = 0; ii < predicates.length; ii++) {
            ...
            if (predicate.type === 'literal' && isNumeric(predicate.value)) {
                ...
                results = await evaluateFilter(predicate, inputSequence, environment);
            }
    ...
    async function evaluateFilter(predicate, input, environment) {
        ...
        for(var index = 0; index < input.length; index++) {
            ...
            var res = await evaluate(predicate, item, environment);
            if (isNumeric(res)) {
                res = [res];
            }
    ...
    async function evaluateBinary(expr, input, environment) {
        ...
        var rhs = await evaluate(expr.rhs, input, environment);
        var op = expr.value;
        ...
        switch (expr.value) {
            ...
            case '[':
                ...
                for(var ii = 0; ii < expr.lhs.length; ii++) {
                    ...
                    var value = await evaluate(item, input, environment);
     ...
     * @returns {*} Evaluated input data
     */
    async function evaluateName(expr, input, environment) {
        ...
        if (Array.isArray(input)) {
            ...
            if(input[expr.value]){
               result = input[expr.value]; 
                ...
                // throw `Relation: ${expr.value} does not exist for Model: ${input.constructor.name}
                //        has relations: ${Object.keys(input.constructor.$$relations).join(",")}`
            }
    ...
    async function evaluateGroupExpression(expr, input, environment) {
        ...
        for(var itemIndex = 0; itemIndex < input.length; itemIndex++) {
            for(var pairIndex = 0; pairIndex < expr.lhs.length; pairIndex++) {
                allPromises.push(await evaluate(expr.lhs[pairIndex][0], input[itemIndex], environment));
            }
        }
        ...
        for (key in groups) {
            ...
            allPromises.push(await evaluate(entry.expr, entry.data, environment));
        }
     ...
     * Used when it's not possible to await
     * @param {Object} expr - AST
     * @param {Object} input - Input data to evaluate against
     * @param {Object} environment - Environment
     * @returns {*} result
     */
    async function driveGenerator(expr, input, environment) {
        var value = await evaluate(expr, input, environment);
        return value;
    }
    ...
    async function evaluateApplyExpression(expr, input, environment) {
        ...
        var arg1 = await evaluate(expr.lhs, input, environment);

        if(expr.rhs.type === 'function') {
            // this is a function _invocation_; invoke it with arg1 at the start
            result = await evaluateFunction(expr.rhs, input, environment, {context: arg1});
        } else {
            ...
            if(isFunction(arg1)) {
                ...
                // λ($f, $g) { λ($x){ $g($f($x)) } }
                result = await apply(chain, [arg1, func], environment, null);
            } else {
                result = await apply(func, [arg1], environment, null);
            }
    ...
    async function evaluateFunction(expr, input, environment, applyto) {
        ...
        for(var ii = 0; ii < expr.arguments.length; ii++) {
            evaluatedArgs.push(await evaluate(expr.arguments[ii], input, environment));
        }
        if(applyto) {
        ...
        // apply the procedure
        try {
            result = await apply(proc, evaluatedArgs, input);
        } catch (err) {
     ...
     */
    async function apply(proc, args, self) {
        try {
            var result;
            result = await applyInner(proc, args, self);
            while(isLambda(result) && result.thunk === true) {
                ...
                result = await applyInner(next, evaluatedArgs, self);
            }
            return result;
        } catch(error){
            throw error;
     ...
     * @returns {*} Result of procedure
     */
    async function applyInner(proc, args, self) {
        var result;
        var validatedArgs = args;
        if(proc) {
            validatedArgs = await validateArguments(proc.signature, args, self);
        }
        if (isLambda(proc)) {
            ...
            result = await proc.apply(self, validatedArgs);
        } else {
            throw {
                code: "T1006",
                stack: (new Error()).stack
     ...
     */
    async function validateArguments(signature, args, context) {
     ...
     * @returns {Array} The array of match objects
     */
    async function functionReplace(str, pattern, replacement, limit) {
        // undefined inputs always return undefined
        if(typeof str === 'undefined') {
            return undefined;
        }
            ...
            if(typeof pattern === 'string') {
                ...
                if (typeof matches !== 'undefined') {
                    while (typeof matches !== 'undefined' && (typeof limit === 'undefined' || count < limit)) {
                        ...
                        var replacedWith = await apply(replacer, [matches], null);
     ...
     */
    async function functionBoolean(arg) {
     ...
     */
    async function functionNot(arg) {
     ...
     */
    async function functionMap(arr, func) {
        // undefined inputs always return undefined
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            // invoke func
            var res = await apply(func, func_args, null);
            if(typeof res !== 'undefined') {
                result.push(res);
            }
        }
    ...
    // This generator function does not have a await(), presumably to make it
     ...
     * @returns {Array} Map array
     */
    async function functionFilter(arr, func) { // eslint-disable-line require-await
        // undefined inputs always return undefined
        if(typeof arr === 'undefined') {
            return undefined;
        }
        ...
        var predicate = async function (value, index, array) {
            var result = await apply(func, [value, index, array], null);
        ...
        for(var i = 0; i < arr.length; i++) {
            var entry = arr[i];
            if(await functionBoolean(await predicate(entry, i, arr))) {
                result.push(entry);
            }
        }
     ...
     * @returns {*} Result
     */
    async function functionFoldLeft(sequence, func, init) {
        // undefined inputs always return undefined
        if(typeof sequence === 'undefined') {
            return undefined;
        }
        ...
        while (index < sequence.length) {
            result = await apply(func, [result, sequence[index]], null);
            index++;
        }
     ...
     */
    async function functionEach(obj, func) {
        var result = [];

        for(var key in obj) {
            var func_args = [obj[key], key];
            // invoke func
            result.push(await apply(func, func_args, null));
        }
     ...
     */
    async function functionSift(arg, func) {
        var result = {};

        var predicate = async function (value, key, object) {
            var result = await apply(func, [value, key, object], null);
            return result;
        };

        for(var item in arg) {
            var entry = arg[item];
            if(await functionBoolean(await predicate(entry, item, arg))) {
                result[item] = await entry;
            }
        }
